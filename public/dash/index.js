...
const views = {
        ...
        ${this.renderDetailSection("Hardware", [
					...
					[
						...
						hit.ip_data?.device?.hardware?.memoryPressure?.used
							? `${(
									hit.ip_data.device.hardware.memoryPressure.used / 1024 / 1024
								).toFixed(2)} MB`
							: undefined,
					],
					[
						...
						hit.ip_data?.device?.hardware?.memoryPressure?.total
							? `${(
									hit.ip_data.device.hardware.memoryPressure.total / 1024 / 1024
								).toFixed(2)} MB`
							: undefined,
					],
					[
						...
						hit.ip_data?.device?.hardware?.memoryPressure?.limit
							? `${(
									hit.ip_data.device.hardware.memoryPressure.limit / 1024 / 1024
								).toFixed(2)} MB`
							: undefined,
					],
					[
        ...
        ${this.renderDetailSection("Storage", [
					[
						...
						hit.ip_data?.device?.storage?.quota
							? `${(
									hit.ip_data.device.storage.quota / 1024 / 1024 / 1024
								).toFixed(2)} GB`
							: undefined,
					],
					[
	...
	async showLinkDetails(slug, preloadedResponse = null) {
      ...
      <div class="link-actions">
        ...
        <button class="action-btn edit" title="Edit destination URL">
          Edit URL
        </button>
        <button class="action-btn delete" title="Delete permanently">
          Delete
        </button>
      </div>        <h1 class="link-title">
    ...
    `;
		...
		$(".edit").addEventListener("click", () =>
			this.showEditForm(slug, response.url, response),
		);
	...
	async showEditForm(slug, currentUrl, preloadedResponse) {
		const template = `
      <div class="confirm-dialog">
        <div class="confirm-content">
          <h1>Edit destination URL</h1>
          <p>Update where this short link redirects to</p>
          ...
          <div class="edit-form">
            <input 
              ...
              value="${utils.escapeHtml(currentUrl)}"
              required
            />
            <div class="edit-actions">
              <button class="save-edit btn-primary">
                Save changes
              </button>
              <button class="cancel-edit btn-secondary">
                Cancel
              </button>
            </div>
          </div>
    ...
    `;
		...
		$(".save-edit").addEventListener("click", () => {
			...
			if (newUrl && utils.isValidUrl(newUrl)) {
				this.handleEditLink(slug, newUrl, preloadedResponse);
			} else {
	...
	async handleEditLink(slug, newUrl, preloadedResponse) {
		$("main").innerHTML = `<div class="loader-wrap">
      <svg width="32" height="32" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><style>.spinner_ajPY{transform-origin:center;animation:spinner_AtaB .4s infinite linear;fill:white}@keyframes spinner_AtaB{100%{transform:rotate(360deg)}}</style><path d="M12,1A11,11,0,1,0,23,12,11,11,0,0,0,12,1Zm0,19a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" opacity=".25" style="fill:white"/><path d="M10.14,1.16a11,11,0,0,0-9,8.92A1.59,1.59,0,0,0,2.46,12,1.52,1.52,0,0,0,4.11,10.7a8,8,0,0,1,6.66-6.61A1.42,1.42,0,0,0,12,2.69h0A1.57,1.57,0,0,0,10.14,1.16Z" class="spinner_ajPY"/></svg>
    </div>`;
		...
		try {
			const editResponse = await utils.apiCall("/api/edit", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
          ...
          <div class="error-display">
            <h2>Error</h2>
            <p>${utils.escapeHtml(editResponse.error)}</p>
            <button class="btn-secondary" onclick="views.showLinkDetails('${slug}', ${JSON.stringify(preloadedResponse)})">
              Go back
            </button>
          </div>
        `;
